...
document.addEventListener('DOMContentLoaded', function() {
  const buttons = document.querySelectorAll('#checkout-button-prompt, #checkout-button-prompt-full');
  buttons.forEach(button => {
    button.addEventListener('click', async function() {
      const {error} = await stripe.redirectToCheckout({
        ...
        cancelUrl: 'https://www.securebydezign.com/articles/prompt-injection.html',
      });
      if (error) {
        console.error(error);
