...
document.addEventListener('DOMContentLoaded', function() {
  const buttons = document.querySelectorAll('#checkout-button-supply_chain_ai, #checkout-button-supply_chain_ai-full');
  buttons.forEach(button => {
    button.addEventListener('click', async function() {
      const {error} = await stripe.redirectToCheckout({
        ...
        cancelUrl: 'https://www.securebydezign.com/articles/supply-chain-ai.html',
      });
      if (error) {
        console.error(error);
