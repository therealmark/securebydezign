...
document.addEventListener('DOMContentLoaded', function() {
  const buttons = document.querySelectorAll('#checkout-button-model_inversion, #checkout-button-model_inversion-full');
  buttons.forEach(button => {
    button.addEventListener('click', async function() {
      const {error} = await stripe.redirectToCheckout({
        ...
        cancelUrl: 'https://www.securebydezign.com/articles/model-inversion.html',
      });
      if (error) {
        console.error(error);
