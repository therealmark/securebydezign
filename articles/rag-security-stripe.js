...
document.addEventListener('DOMContentLoaded', function() {
  const buttons = document.querySelectorAll('#checkout-button-rag_security, #checkout-button-rag_security-full');
  buttons.forEach(button => {
    button.addEventListener('click', async function() {
      const {error} = await stripe.redirectToCheckout({
        ...
        cancelUrl: 'https://www.securebydezign.com/articles/rag-security.html',
      });
      if (error) {
        console.error(error);
