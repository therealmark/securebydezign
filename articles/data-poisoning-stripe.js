...
document.addEventListener('DOMContentLoaded', function() {
  const buttons = document.querySelectorAll('#checkout-button-data, #checkout-button-data-full');
  buttons.forEach(button => {
    button.addEventListener('click', async function() {
      const {error} = await stripe.redirectToCheckout({
        ...
        cancelUrl: 'https://www.securebydezign.com/articles/data-poisoning.html',
      });
      if (error) {
        console.error(error);
